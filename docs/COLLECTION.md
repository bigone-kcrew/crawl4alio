...
### ALIO 법령/지침 게시판 — `collect_alio_lawboard.js`

ALIO `etc/etcLawList.do` 게시판(기재부 지침류 개정 게시)을 수집합니다. 예산운용지침·통합공시기준·혁신지침·안전관리지침 등 **같은 지침의 개정 이력이 개별 게시글**로 쌓여 있어(2018~현재, 78건+) 개정 추적 소스로도 유용합니다.

```bash
node collection/collect_alio_lawboard.js --dry-run          # 목록·첨부 미리보기
node collection/collect_alio_lawboard.js                    # 수집 (boardNo ckpt 증분)
```

- API: `GET /etc/findEtcLawList.json?type=title&word=&pageNo=N` → `GET /etc/findEtcLawDtl.json?boardNo=` → `download.json?fileNo=` (본문 없음, 첨부 게시용)
- 저장: 첨부 → `<legal-raw>/법령자료/알리오지침게시판/<게시글>/`, manifest → `<legal-md>/…` (raw/md 원칙)
- 체크포인트: `logs/lawboard_ckpt.json` (boardNo+idate, 게시 수정 시 재처리)
