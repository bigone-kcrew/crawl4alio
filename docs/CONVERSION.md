...
수집~OCR을 사람 없이 굴리기 위한 감독 스크립트 3종입니다. 모두 env로만 구동되며(하드코딩 없음), 실운영에서 크래시·정체·오발동을 겪고 고친 로직 그대로입니다.

| 스크립트 | 역할 |
|---|---|
| `scripts/ocr_watchdog.sh` | OCR 인스턴스 감독 — 사망 재기동·정체(로그 mtime) 감지·큐 소진 백오프·재배분 config 반영 |
| `scripts/ocr_rebalance.sh` | safe(저RAM) 인스턴스 소진 임박 시 `OCR_SPLIT_PAGES` 상향 → 저밀도 대형을 나눠 갖기. 밀도 임계 불변·SPLIT 상향만·서버 정상 시에만 등 안정성 불변식 내장 |
| `scripts/recover_then_reprocess.sh` | 회수 → (REPROCESS=1) 재처리 체인. cron의 변환 후·OCR 전 단계에 삽입 |

```bash
# 듀얼 PC 예시 — 각 머신(또는 한 머신에서 두 프로세스)
INSTANCE=pc1 CATALOG_ROOT=/data OCR_BAND=risky OCR_SPLIT_PAGES=72 STALL_SEC=2400 \
  PADDLEOCR_PARSE_URL=http://PC1:13430/parse bash scripts/ocr_watchdog.sh &
INSTANCE=pc2 CATALOG_ROOT=/data OCR_BAND=safe OCR_SPLIT_PAGES=72 OCR_CHUNK_PAGES=6 OCR_MAX_TIMEOUT=480000 \
  PADDLEOCR_PARSE_URL=http://PC2:13430/parse bash scripts/ocr_watchdog.sh &
...
> 워치독의 정체 판정은 **실행 로그 mtime** 기준입니다(청크마다 갱신). inflight 파일 기준으로 하면 20분+ 걸리는 대형 다청크 문서를 "정체"로 오인해 죽입니다 — 겪고 고친 함정. 대형 담당 인스턴스는 `STALL_SEC=2400` 권장.
