...
// 게시글(posting) 동시 처리 수 — 순차 오버헤드(상세조회+sleep, 전체의 65%) 압축용
const POSTING_CONCURRENCY = Number(process.env.RECRUIT_POSTING_CONCURRENCY || 6);
...
// 전역 세마포어: 동시 실행을 max개로 제한. slot은 단일 요청 동안만 점유하므로
// 중첩 acquire가 없어 데드락 없음.
...
// alio.go.kr 요청 전역 리미터 (상세조회 + 파일다운로드가 공유)
...
// 스트림 다운로드 — 파일 전체를 메모리에 버퍼링하지 않음(구 arraybuffer 방식 교체).
// 높은 동시성에서 RAM 스파이크 방지. 요청은 alioSem으로 전역 제한.
...
async function processForm(inst, formNo, args, cutoffYear, ckpt, totals) {
    ...
    const processPosting = async (row) => {
        ...
        if (ckpt.done[key]) {
            ...
            if (!saved.idate || saved.idate === (row.idate || '')) { totals.skipped += 1; return; }
...
if (require.main === module) {
    ...
    module.exports = { runPool, acquireFile, fileNoCache, createSemaphore, downloadFile, alioSem };
}
