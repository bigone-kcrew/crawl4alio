...
### Changed
- **텔레그램 알림 문구 개선(오해 방지)** (`n8n/workflows/ALIO_수시감지·정기감지·완료통지_v1.json`): 감지/완료 알림의 `digest.failed`(코퍼스 **누적** 변환실패)를 헤드라인 "실패 N"에서 분리 → **"ℹ 코퍼스 누적 변환실패 N건(대부분 DRM·미지원·참고, 이번 감지와 무관)"**으로 명확히 라벨. 매 실행 실패처럼 보이던 오해 제거.
- **이번 실행 실제 오류 표면화**: 알림이 새 digest 필드 `run_errors`(해당 실행 로그의 개별 상세조회/다운로드 실패 등)를 참조 — 0 초과 시 "⚠ 이번 실행 오류 N건 — 로그 확인"을 헤드라인·본문에 부각. 크래시(exit≠0) 오류 브랜치는 그대로 유지(진짜 실패 누락 방지). `run_errors`는 정기 러너 digest에서 산출(운영측).
...
### Changed
- **게시판형 수집 대상에서 연구보고서(B1040·B1260) 제외**: minor_category=`정보공개`로 노동·평가·ESG 수집 스코프 밖(계약정보 B1030을 뺀 것과 동일 기준). v1.9.7에서 일괄 추가했던 5종 중 연구보고서 2종만 제거. 유지: B1240 고객만족도(평가)·B1280 복리후생(핵심)·B1270 환경보호(ESG). FORM_CONFIG 7종.
