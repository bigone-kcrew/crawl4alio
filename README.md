...
| 스캔 PDF OCR | ❌ `ocr_needed` 큐 대기 | ✅ PaddleOCR |
...
cd deploy && docker compose up -d && cd .. # (풀스택) 파서 스택 기동
...
| `npm run convert:ocr` | 스캔 문서 OCR(PaddleOCR) |
...
- **raw/md 트리 분리는 모든 수집·변환기가 지켜야 한다.** 일부만 지키면 검색·RAG가 본문을 못 보거나 raw 오프사이트 삭제가 막힌다. 변환 산출(.md)도 반드시 md 트리로(`convert_ocr_needed`의 misroute 수정, v1.4.0).
