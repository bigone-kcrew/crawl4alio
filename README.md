...
**OCR 스케일아웃 (여러 CPU에 분산)** — 밴드(크기·밀도)·페이지 균형점·샤딩·인스턴스별 체크포인트 env로 약한 CPU 여러 대에 스캔 OCR을 분산(메모리 안전+속도 균형+무인 자기치유). 상세 노브·예시는 **[docs/OCR_SCALEOUT.md](docs/OCR_SCALEOUT.md)**.
