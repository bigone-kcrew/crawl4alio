...
`node rag/mcp_server.js` — 무의존 stdio JSON-RPC. 도구: `search_corpus`(keyword/semantic/hybrid + 필터), `get_document`, `get_article`. 검색 결과의 `doc_id`(=`corpus:rel_path`)로 상세 조회 연계. 등록만 해두면 세션 시작/재시작 시 자동으로 물려 세션 내내 재사용된다(질의마다 재스폰 아님).

### 등록

**Claude Code** (user 스코프 — 모든 프로젝트에서 사용):
```bash
claude mcp add -s user alio-rag -e RAG_ROOT=/path/to/workspace -- node <repo>/rag/mcp_server.js
claude mcp list   # alio-rag … ✔ Connected 확인
```

**Codex** — `codex mcp add` 후 서버 테이블에 승인 모드를 지정해야 한다:
```bash
codex mcp add alio-rag -- node <repo>/rag/mcp_server.js
```
```toml
# ~/.codex/config.toml
[mcp_servers.alio-rag]
command = "node"
args = ["<repo>/rag/mcp_server.js"]
default_tools_approval_mode = "approve"   # ← 필수
```
> ⚠️ **`default_tools_approval_mode = "approve"`가 없으면** `approval_policy=never` + read-only 샌드박스에서 도구 승인 UI를 띄울 수 없어 호출이 **즉시 `user cancelled MCP tool call`로 취소**된다(네트워크 문제 아님). stdio MCP는 샌드박스 밖 로컬 프로세스로 실행되므로 네트워크 차단과 무관하다.

### 집계형 질문 팁

`search_corpus`는 기본이 **제목 검색(`body=false`)**이라 "승진 규정 몇 개?" 같은 개수/전수 질문에 과소집계되기 쉽다. 이런 질문엔 **`body=true`로 본문까지 검색**하고 `limit`을 넉넉히 둔 뒤, **"조문 수"와 "고유 문서 수"(`doc_id` 기준)를 구분**해 세라. (도구 description에도 동일 힌트가 들어 있다.)
